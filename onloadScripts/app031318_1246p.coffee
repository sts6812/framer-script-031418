...
window.onload  = ->
	...
	for layers, i in eval(target)
		...
		if options.source.value==true
			...
			Rotation = jsonLayer.rotation
			scaleX = jsonLayer.scaleX
			scaleY = jsonLayer.scaleY
	...
	$(document).keypress (event) -> 
		...
		if keycode == 13
			...
			for layer, i in buttonArray
				console.log buttonArray[i].stateCycle()
